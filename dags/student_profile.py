...
def update_student_profile_record():
    try:
        # Set up logging
        ...
        # Open the Google Sheet 'Student Profile'
        logging.info("Opening Google Sheet 'Student Profile'...")
        workbook = client.open("Student_Profile")
        sheet = workbook.sheet1  # Assuming you're working with the first sheet

        # Fetch the data range A1:Q41 from the 'Student Profile' sheet
        logging.info("Fetching data range A1:Q41 from the Google Sheet...")
        range_data = sheet.get('A1:Q41')  # Fetch the specified range
        ...
        # Check if the 'Student_Profile' table already exists
        logging.info("Checking if 'Student_Profile' table exists in the database...")
        table_exists = con.execute("SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'Student_Profile'").fetchone()[0]

        # Step 2: Fetch existing data
        existing_df = con.execute("SELECT * FROM Student_Profile").fetchdf() if table_exists else pd.DataFrame()
        ...
        if not existing_df.empty:
            ...
            if not new_or_updated_rows.empty:
                ...
                con.execute("DROP TABLE IF EXISTS Student_Profile;")
                logging.info("Old table dropped. Re-creating 'Student_Profile' table with updated data...")

                # Re-create the table
                create_sql = create_table_sql("Student_Profile", df_new)
                con.execute(create_sql)

                # Insert all data (including new and updated rows)
                placeholders = ', '.join(['?' for _ in df_new.columns])
                insert_sql = f"INSERT INTO Student_Profile VALUES ({placeholders})"

                for i, row in df_new.iterrows():
                    con.execute(insert_sql, tuple(row))  # Insert each row

                logging.info(f"All data (including updated rows) successfully inserted into 'Student_Profile'.")
            else:
            ...
            logging.info("All rows successfully inserted into 'Student_Profile'.")
